  ...
  if (nowMs - lastSampleMs >= SAMPLE_MS) {
    ...
    if (beaconPresent && !lastPresent) {
      ...
      if (sinceTrigger < LAP_COOLDOWN_MS) {
        ...
        uint32_t sinceLastLap = (last_lap_ms == 0) ? 999999UL : (nowMs - last_lap_ms);

        if (sinceLastLap >= MIN_LAP_MS && session_state == STATE_RUNNING) {
          uint32_t lap_time_ms = nowMs - last_lap_ms;
          lap_count++;
          last_lap_ms = nowMs;
          ...
          const char* reason = (session_state != STATE_RUNNING) ? "state" : "minlap";
          Serial.printf("IGNORE @ %lu ms (%s) falls=%u\n",
                        (unsigned long)nowMs, reason, (unsigned)fallsLast);
        }
      }
    }
